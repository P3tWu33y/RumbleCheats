...
void utils::unload_dll_by_name(const char* dllName) {
    // Get current module handle (to avoid self-unloading)
    HMODULE hCurrentModule = nullptr;
    GetModuleHandleExA(
        GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
        GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
        reinterpret_cast<LPCSTR>(&unload_dll_by_name),
        &hCurrentModule
    );

    HMODULE hTarget = nullptr;
    while ((hTarget = GetModuleHandleA(dllName))) {
        // Critical: Prevent unloading our own module
        if (hTarget == hCurrentModule) {
            std::cerr << "Blocked self-unload: " << dllName << "\n";
            break;
        }

        if (!FreeLibrary(hTarget)) {
            std::cerr << "FreeLibrary failed (" << dllName
                << "). Error: " << GetLastError() << "\n";
            break;
        }
    }
